...
from contracts.interfaces.Ivault_factory import Ivault_factory
...
@storage_var
func shares_total_suply() -> (res: Uint256):
end
...
namespace FuccountLib:
    ...
    func get_shares_total_suply{
        pedersen_ptr: HashBuiltin*, 
        syscall_ptr: felt*, 
        range_check_ptr
    }() -> (totalSupply: Uint256):
    let (totalSupply: Uint256) = shares_total_suply.read()
    return (totalSupply)
    end
...
func get_share_price{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (total_supply) = shares_total_suply.read()
...
func preview_reedem{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (is_available_reedem) = is_available_reedem(amount)
    with_attr error_message("preview_reedem: Not enought liquid positions"):
            assert is_available_reedem = 1
    end
...
func mint{
        ...
        _share_price_purchased:Uint256,
    ):
    let (total_id_) = total_id.read()
    share_price_purchased.write(total_id_, _share_price_purchased)
    let (currentTimesTamp_:felt) = get_block_timestamp()
    minted_block_timestamp.write(total_id_, currentTimesTamp_)
    let (currentTotalSupply_) = shares_total_suply.read()
    let (newTotalSupply_,_) = uint256_add(currentTotalSupply_, sharesAmount )
    shares_total_suply.write(newTotalSupply_)
    let (newtotal_id_,_) = uint256_add(total_id_, Uint256(1,0) )
    total_id.write(newtotal_id_)
    _mint(to, total_id_, sharesAmount)
    return ()
end
...
func _check_call{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (integrationManager_:felt) = Ivault_factory.getIntegrationManager(vault_factory_)
...
func _calc_amount_of_each_asset{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    if isAssetFundBalanceNul_ == 1:
        ...
        let (assetvalueInDeno_: Uint256) = get_asset_value(asset[0], assetFundbalance_, denomination_asset_)
        let (isAssetFundBalanceEnought: felt) = uint256_le(totalValue, assetvalueInDeno_)
        if isAssetFundBalanceEnought == 1:
            let (requiredAmount: Uint256) = get_asset_value(denomination_asset_, totalValue, asset[0])
...
func _calc_amount_of_each_share{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    if isShareFundBalanceNul_ == 1:
        ...
        let (sharevalueInDeno_: Uint256) = getShareValue(share[0].address, share[0].id,shareFundbalance_, denomination_asset_)
        let (isShareFundBalanceEnought: felt) = uint256_le(totalValue, sharevalueInDeno_)
        if isShareFundBalanceEnought == 1:
            let (oneSharevalueInDeno_: Uint256) = getShareValue(share[0].address, share[0].id,Uint256(POW18,0), denomination_asset_)
...
func _get_dao_treasury{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (treasury_:felt) = Ivault_factory.getDaoTreasury(vault_factory_)
...
func _get_stacking_vault{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (stackingVault_:felt) = Ivault_factory.getStackingVault(vault_factory_)
...
func _get_dao_treasury_fee{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (treasury_:felt) = Ivault_factory.getDaoTreasuryFee(vault_factory_)
    return (res=treasury_)
end
...
func _get_stacking_vault_fee{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (stackingVault_:felt) = Ivault_factory.getStackingVaultFee(vault_factory_)
    return (res=stackingVault_)
end
...
func _get_fee_manager{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (feeManager_:felt) = Ivault_factory.getFeeManager(vault_factory_)
...
func _get_policy_manager{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (policyManager_:felt) = Ivault_factory.getPolicyManager(vault_factory_)
...
func _get_value_interpretor{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    let (vault_factory_:felt) = vault_factory.read()
    let (valueInterpretor_:felt) = Ivault_factory.getValueInterpretor(vault_factory_)
...
func _assert_enought_guarantee{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    alloc_locals
   let (shareSupply_) = shares_total_suply.read()
   let (contractAddress_ : felt) = get_contract_address()
   let (vault_factory_) = vault_factory.read()
   let (stackingDispute_) = Ivault_factory.getStackingDispute(vault_factory_)
   let (securityFundBalance_)  = IStackingDispute.getSecurityFundBalance(stackingDispute_, contractAddress_)
   let (guaranteeRatio_) = Ivault_factory.getGuaranteeRatio(vault_factory_)
...
func _complete_non_nul_asset_tab{
    ...
    let (assetId_len:felt, assetId:Uint256*, assetAmount_len:felt,assetAmount:Uint256*) = IFuccount.owner_shares(fundAddress_, selfAddress)
    if assetId_len == 0:
