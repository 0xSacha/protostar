...
from interfaces.IJediSwapPair import IJediSwapPair, IJediSwapPairERC20
...
func calcUnderlyingValues{
    ...
    let (poolAllocation_:felt) = uint256_div(amountMul_, _amount)
    let (reserveToken0_:Uint256, reserveToken1_:Uint256,_) = IJediSwapPair.get_reserves(_derivative)
    ...
    let (underlyingAmount0_:Uint256) = uint256_div(underlyingAmount0Mul_, PRECISION)
    let (underlyingAmount1_:Uint256) = uint256_div(underlyingAmount1Mul_, PRECISION)
